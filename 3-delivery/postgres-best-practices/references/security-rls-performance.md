...
> (Supabase-specific) Examples below use Supabase's `auth.uid()` helper and built-in roles (`anon`, `authenticated`, `service_role`). The pattern — wrap per-row function calls in a scalar `(select ...)` so they evaluate once — applies to any function in a policy on vanilla Postgres (e.g. `current_setting('app.current_user_id')`).
