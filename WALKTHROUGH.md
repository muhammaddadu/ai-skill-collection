...
| 13. Attack surface? | ▸ `threat-modeling` | New data class (theme preference on the profile) → short-pass STRIDE: enumeration risk on the profile endpoint noted, mitigation lands in the spec | Threat model, mitigations → spec |
| 14. API impact? | ↩ ▸ `api-design` | Only if the backend stores a theme preference — here: one field on the profile endpoint, checked against A20 (no breaking change) | Contract note in the tech spec |
| 15. Engineering design | ▸ `tech-spec` | The PRD's "how": chosen design, data impact, test approach, rollout sketch | `docs/specs/dark-mode.md` |
| 16. Business rules | ▸ `prd-development` (Norming) | Stakeholders validate business rules (default theme, brand palette) — **not** the user journey | PRD (Normed) |
...
2. **Discovery** — `prd-development` × (`design-signal` 30/60 ∥ `eng-feasibility-spike` → `adr` → `tech-spec` (+ `threat-modeling`, `api-design`)) → ✋ Sign-Off 2
3. **Delivery** — `pi-planning` → stories (`epic-breakdown-advisor`, `user-story`) → `test-strategy` → build (`tdd`, `cleanui`, stack skills, `e2e-automation`) → `web-design-guidelines` → `slo-design` → `progressive-delivery` → `release-readiness` → ship → flag cleanup
